...
public class Save {
    ...
    public static void recentWords(Search recent, String searchString) {
        JSONObject jsonRecentWord = new JSONObject();
        jsonRecentWord.put(recent.getCurentWord(), searchString);
        List<String> recentWordsList = new ArrayList<>(Arrays.asList(jsonRecentWordsList));
        recentWordsList.add(jsonRecentWord.toJSONString());
        ...
        try {
            ...
            FileWriter file = new FileWriter("D:/recentWords.json");
        ...
        System.out.println("JSON file created: "+jsonRecentWord);
    }
}
